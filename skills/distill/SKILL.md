...
- `memory_store_resource` — models: Person, Project, Company, Task, Technology, Decision, Pattern. Every resource needs `name`; other camelCase properties are free-form. Decisions: add `rationale`, `outcome`, `date`. Patterns (gotchas/approaches): add `description`, optionally `example`. Upserts by model+name — no need to check existence first, but only set properties you have real content for.
