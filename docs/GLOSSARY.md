...
- **Structured context entry** — a context-file bullet plus indented `key: value` lines
  mirroring the graph shape (properties, `relation: Model/name` links, `concepts:`). Written
...
- **Stop block** — the context-write trigger's channel: when counted activity outruns the
  context log, the Stop hook returns `{"decision": "block", "reason": …}` and the model must
...
## Code knowledge (rules live; code graph horizon)

- **Semantic overlay** — the *why* of code (decisions, gotchas, constraints) — belongs in
  memory; small and durable.
- **Orientation memory** — the *map*: how and where things are stored, located, and wired at
  convention level (state layout, persistence formats, subsystem wiring). A Pattern with
  `kind: layout|storage|wiring`; keep test in
  [tasks/code-memory-rules](tasks/code-memory-rules.md).
- **Trace (investigation finding)** — the answer a long dig paid for (a path/flow across
  files), stored as a Pattern `kind: trace` with the *question phrasings as aliases* and a
  mandatory anchor. **Cost-gated, not topic-gated**: one grep answers it → stays out; many
  greps answered it → the finding earns a node. Detection is the dig counter's job.
- **`manifestsIn`** — the relation from a *why* to its *where*: Decision manifestsIn the
  anchored layout/trace Pattern(s) where the choice lives in code, one per project. File
  paths belong in those Patterns (anchorable, drift-flaggable), never in Decision properties.
- **Bulk structure** — symbol-level facts (definitions, call sites). Never *authored*: the
  noise argument is volume, not content — thousands of enumerated symbols would flatten term
  distinctiveness. Authored nodes exist because a session *paid* for them.
- **Derived code graph** — bulk structure done right: generated by tree-sitter/SCIP into a
  separate named graph, regenerated per commit, never hand-written (horizon).
